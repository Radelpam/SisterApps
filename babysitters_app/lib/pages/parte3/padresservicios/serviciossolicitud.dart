...
class _ServiciosSolicitudState extends State<ServiciosSolicitud> {
  ...
  void _onSelectionChanged(DateRangePickerSelectionChangedArgs args) {
    setState(() {
      if (args.value is PickerDateRange) {
        _range1 = '${nt.DateFormat('dd/MM/yyyy').format(args.value.startDate)}';
        _range2 =
            ' ${nt.DateFormat('dd/MM/yyyy').format(args.value.endDate ?? args.value.startDate)}';
        if (args.value.endDate != null) {
          cantidad = (args.value.endDate.day - args.value.startDate.day);
        }
  ...
  tim() async {
    ...
    var temp;
    var timeaact = (TimeOfDay(hour: 12, minute: 30).hour + 2);
          ...
          child: Center(
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  Text(
                    "Total precio: ${formatCurrency.format(total * cantidad)}",
  ...
  Future<void> addUser() async {
    ...
    await users.add({
      ...
      'precio': (widget.preciofinal * cantidad),
      'observaciones': observacionesController.text,
      'Hora': horainicio,
      'DiasTotal': cantidad,
      'fechainicial': _range1
    }).then((value) async {
      await userdata
          .doc(FirebaseAuth.instance.currentUser!.uid)
          .update({'esperando': true}).then((value) {
